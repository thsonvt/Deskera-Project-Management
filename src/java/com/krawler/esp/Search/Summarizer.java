...
import com.krawler.esp.Search.Summary.Ellipsis;
import com.krawler.esp.Search.Summary.Fragment;
import com.krawler.esp.Search.Summary.Highlight;
...
public class Summarizer {
	...
	private static final int SUM_CONTEXT = Integer
			.parseInt(com.krawler.esp.handlers.StorageHandler
					.GetSummaryContext("5"));
	private static final int SUM_LENGTH = Integer
			.parseInt(com.krawler.esp.handlers.StorageHandler
					.GetSummaryLength("35"));
			...
			if (highlight.contains(tokens[i].termText())) {
				...
				while ((j < endToken) && (j - startToken < SUM_LENGTH)) {
					...
					if (highlight.contains(t.termText())) {
						...
						excerpt.add(new Fragment(text.substring(offset, t
								.startOffset())));
						excerpt.add(new Highlight(text.substring(t
				...
				//
				if (j < tokens.length) {
					excerpt.add(new Fragment(text.substring(offset, tokens[j]
							.endOffset())));
				}
