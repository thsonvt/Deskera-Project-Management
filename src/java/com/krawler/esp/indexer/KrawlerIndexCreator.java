...
public class KrawlerIndexCreator {
	...
	public KrawlerIndexCreator() throws ServiceException {
		try {
			this.indexPath = com.krawler.esp.handlers.StorageHandler
					.GetDocIndexPath();
	...
	public int CreateIndex(ArrayList<DocumentFields> DocFields) {
		...
		while (itr.hasNext()) {
			...
			Field docfield = new Field(tempfield.GetFieldName(), tempfield
					.GetFieldValue(), Field.Store.YES, Field.Index.TOKENIZED);
			doc.add(docfield);
		}
