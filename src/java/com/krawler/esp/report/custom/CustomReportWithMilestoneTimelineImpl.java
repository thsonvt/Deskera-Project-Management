 ...
 * @author Abhay
 */
public class CustomReportWithMilestoneTimelineImpl implements CustomReport {

    private String companyId, loginID;
    ReportQueryBuilder qb;
    private boolean isPaging;
    private boolean isMilestone;

    public CustomReportWithMilestoneTimelineImpl(String companyId, String userID, boolean isPaging, boolean isMilestone) {
    ...
    public CustomReportWithMilestoneTimelineImpl(String companyId, String loginID) {
    ...
    public int insertReport(Connection conn, String reportId, String reportname, String reportType, String description, String userid) throws ServiceException {
        ...
        String query = "insert into customreports(reportid,companyid,reportname,type,description,category,createdby,createdon,modifiedon,paging,milestone) values(?,?,?,?,?,'Custom',?,now(),now(),?,?)";
        return DbUtil.executeUpdate(conn, query, new Object[]{reportId, companyId, reportname, reportType, description, userid, isPaging, isMilestone});
    }
    ...
    public ReportBean getReportData(Connection conn, ReportQueryBuilder query) throws ServiceException {
        ...
        boolean permission = pm.isPermission(conn, loginID, PermissionConstants.Feature.CUSTOM_REPORTS, PermissionConstants.Activity.CUSTOM_REPORT_VIEW);
        try {
            int count = 0;
            if (permission) {
                ps = conn.prepareStatement(query.getCountQuery());
            } else {
                ps = conn.prepareStatement(query.getCountQueryForMT(loginID));
            }
            ...
            if (permission) {
                ...
                ps = conn.prepareStatement(query.getQueryForMT(loginID));
            }
            ps.setString(1, companyId);
            rs = ps.executeQuery();
