...
public class ByteUtil {
	...
	public static byte[] getContent(File file) throws IOException {
		...
		try {
			...
			while (num_left > 0
					&& (num_read = is.read(buffer, total_read, num_left)) != -1) {
				total_read += num_read;
				num_left -= num_read;
			}
		} finally {
	 ...
	 *            <tt>byte[]</tt> array.
	 */
	public static byte[] getContent(InputStream is, int sizeHint)
			throws IOException {
	 ...
	 */
	public static byte[] getContent(InputStream is, int sizeHint, long sizeLimit)
			throws IOException {
	 ...
	 *            <tt>byte[]</tt> array.
	 */
	public static byte[] getPartialContent(InputStream is, int length,
			int sizeHint) throws IOException {
		return getContent(is, length, sizeHint, -1);
	}

	private static byte[] getContent(InputStream is, int length, int sizeHint,
		...
		try {
			...
			int num, limit = length > 0 ? Math.min(buffer.length, length
					- baos.size()) : buffer.length;
			while ((num = is.read(buffer, 0, limit)) != -1) {
				...
				limit = length > 0 ? Math.min(buffer.length, length
						- baos.size()) : buffer.length;
			}
	...
	public static void closeStream(InputStream is) {
		...
		try {
			...
			KrawlerLog.misc.debug(
					"ignoring exception while closing input stream", e);
		}
	}
	...
	public static void closeStream(OutputStream os) {
		...
		try {
			...
			KrawlerLog.misc.debug(
					"ignoring exception while closing output stream", e);
		}
	}
	 ...
	 * @throws IOException
	 */
	public static int copy(InputStream in, boolean closeIn, OutputStream out,
	 ...
	 */
	public static int copy(InputStream in, boolean closeIn, OutputStream out,
			boolean closeOut, long maxLength) throws IOException {
		try {
			byte buffer[] = new byte[8192];
			int numRead;
			int transferred = 0;
			while ((numRead = in.read(buffer)) >= 0) {
				...
				if (maxLength >= 0 && transferred > maxLength)
					throw new IOException("stream exceeded allowable length: "
							+ transferred);
			}
			return transferred;
		} finally {
	...
	public static void writeUTF8(DataOutput out, String str) throws IOException {
		...
		if (len > MAX_STRING_LEN)
			throw new IOException("String length " + len
					+ " is too long in ByteUtil.writeUTF8(); max="
					+ MAX_STRING_LEN);
		if (len > 0) {
	...
	public static class SegmentInputStream extends PositionInputStream {
		...
		public static SegmentInputStream create(InputStream is, long start,
		...
		@Override
		public int read(byte[] b, int off, int len) throws IOException {
			return available() <= 0 ? -1 : super.read(b, off, Math.min(len,
					available()));
		}
